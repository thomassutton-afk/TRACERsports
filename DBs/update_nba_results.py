#!/usr/bin/env python3
"""
update_nba_results.py

Fills PF / PA / OT into NBA_2026_Results.xlsx by pulling final scores from
...
USAGE
    python update_nba_results.py --file NBA_2026_Results.xlsx --start 2026-10-21 --end 2026-10-27
    python update_nba_results.py --file NBA_2026_Results.xlsx --date 2026-10-21
    python update_nba_results.py --file NBA_2026_Results.xlsx --start 2026-10-21 --end today
...
NOTES ON THE DATA SOURCE
    ...
    (not city) against the TEAM_NAME_MAP below — NBA has two cities that
    each host two teams (Los Angeles: Lakers/Clippers, New York: only the
    Knicks, so that one's fine, but LA still isn't), so a city-substring
    match would be ambiguous the way it isn't for a single-team-per-city
    league. Run with --diagnose-date on a real game day to print ESPN's
    raw team names before trusting the mapping, and adjust TEAM_NAME_MAP
    if a team fails to match.
"""
...
# of the 2026 season's 30 active franchises.
TEAM_NAME_MAP = {
    "hawks": "ATL",
    "celtics": "BOS",
    "nets": "BRK",
...
def fetch_scores_for_date(d: date) -> list[dict]:
    ...
    for event in data.get("events", []):
        ...
        # NBA regulation is 4 quarters; period > 4 means OT was played.
...
def main():
    ...
    ap.add_argument("--file", required=True, help="Path to NBA_2026_Results.xlsx")
