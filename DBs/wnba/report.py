...
Produces, in a `reports/` folder next to the database:
    standings.csv     - one row per team per season (W/L, final Elo, rank)
...
reflects whatever is currently in wnba_elo.db.
"""
import csv
import os
import db
...
def write_standings_csv(conn, path):
    ...
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["season", "rank", "team_id", "team_name", "wins", "losses", "final_elo"])
        for season in seasons:
            rows = standings(conn, season)  # already ranked by final_elo desc
            for rank, (team_id, name, wins, losses, rating) in enumerate(rows, start=1):
                w.writerow([season, rank, team_id, name, int(wins), int(losses), round(rating, 2)])


def write_game_log_csv(conn, path):
    cols = ["date", "season", "type", "round", "team", "opponent", "home_away",
            "points_for", "points_against", "ot", "pre_rate", "post_rate",
            "rating_change", "expected_win", "result"]
    rows = conn.execute(
...
def write_summary_txt(conn, path):
    seasons = [r[0] for r in conn.execute("SELECT DISTINCT season FROM games ORDER BY season")]
    lines = []
    lines.append("WNBA Echo Ratings - Standings Summary")
    lines.append("=" * 40)
    for season in seasons:
        ...
        lines.append("-" * 40)
        for rank, (team_id, name, wins, losses, rating) in enumerate(rows, start=1):
            lines.append(f"{rank:2d}. {name:24s} {int(wins):>2d}-{int(losses):<2d}   Elo {rating:8.2f}")
