#!/usr/bin/env python3
"""
update_nfl_results.py

Fills PF / PA / OT into NFL_2026_Results.xlsx by pulling final scores from
...
USAGE
    python update_nfl_results.py --file NFL_2026_Results.xlsx --start 2026-09-10 --end 2026-09-16
    python update_nfl_results.py --file NFL_2026_Results.xlsx --date 2026-09-10
    python update_nfl_results.py --file NFL_2026_Results.xlsx --start 2026-09-10 --end today
...
NOTES ON THE DATA SOURCE
    ESPN's endpoint is unofficial, so team names are matched by NICKNAME
    (not city) against the TEAM_NAME_MAP below — NFL has two cities that
    each host two teams (Los Angeles: Rams/Chargers, New York: Giants/
    Jets), so a city-substring match would be ambiguous the way it isn't
    for a single-team-per-city league. Run with --diagnose-date on a real
    game day to print ESPN's raw team names before trusting the mapping,
    and adjust TEAM_NAME_MAP if a team fails to match.
"""
...
# Map substrings found in ESPN's team display name -> the code used in the
# sheet. Matched on NICKNAME, not city (see module docstring) - current as
# of the 2026 season's 32 active franchises.
TEAM_NAME_MAP = {
    "cardinals": "ARI",
    "falcons": "ATL",
    "ravens": "BAL",
...
def fetch_scores_for_date(d: date) -> list[dict]:
    ...
    for event in data.get("events", []):
        ...
        # NFL regulation is 4 quarters; period > 4 means OT was played.
...
def main():
    ...
    ap.add_argument("--file", required=True, help="Path to NFL_2026_Results.xlsx")
    ...
    c_pf = col_index(ws, "PointsFor")
    c_pa = col_index(ws, "PointsAgainst")
