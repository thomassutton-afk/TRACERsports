...
// table before any real games for it actually exist. Trusting that row
// as-is picks a season with zero games - every query scoped to it then
// legitimately returns nothing, which is exactly what happened on the
// homepage for NFL (2026 preseason row present, but NFL's real games
// only go through 2025 while its season is still in progress). This
// confirms each candidate actually has game rows before accepting it,
// falling back to the next most recent season otherwise.
export async function getCurrentSeason(league) {
  ...
  for (const season of seasons) {
    const { count, error: checkError } = await supabase
      ...
      .eq("variant", "echo")
      .limit(1);
    if (!checkError && count > 0) return { season, error: null };
  }
