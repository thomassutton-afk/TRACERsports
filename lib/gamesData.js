...
export function pairScheduleRows(rows, nextGameByTeam) {
  ...
  for (const row of rows) {
    ...
    games.push({
      ...
      away: row.opponent_id,
      winProb: row.expected_win_pct,
      // Home-perspective, matches the sign convention already used by
      // spread.py/predict.py: positive = home favored by that many
      // points, negative = away favored. Null until a league's
      // export pipeline actually writes it (currently NFL only).
...
export async function getScheduledGamesForDate(league, dateStr, season, variant) {
  const [{ data }, nextGameByTeam] = await Promise.all([
    supabase
      .from("schedule")
      .select("team_id,opponent_id,home_away,date,type,round,expected_win_pct,predicted_spread")
      .eq("league", league)
      .eq("season", season)
      .eq("variant", variant)
      .eq("date", dateStr),
...
// Latest PLAYED game date for a season, so GamesPanel can default to
// "show me the most recent action" instead of making the user pick a
// date on first load. Falls back to the EARLIEST scheduled date when
// there's no played game yet (a season in its preseason phase, like
// right before Week 1 kicks off) - the alternative is returning null,
// which leaves the date picker empty and the panel stuck on
// "Loading..." forever, since nothing ever supplies a date to fetch.
// Landing on day 1 of the schedule is a much more useful default than
// a blank panel for a season that hasn't started yet.
...
//
// Reads candidate seasons from preseason_ratings (small table, cheap to
// query - see getAvailableSeasons above), but does NOT trust the top
// candidate blindly: preseason_ratings is written with ON CONFLICT DO
// UPDATE rather than a delete-first export step (see HANDOFF), so a
// season-rollover export can leave a "next season" row sitting in that
// table before add_season.py has ever run for it - no schedule, no
// games, nothing else about it real yet. Trusting that row as-is picks
// a season with nothing behind it at all.
//
// The distinguishing signal isn't "has a completed game" - a season is
// legitimately current for its entire preseason too, which is exactly
// when showing it matters most (that's the whole point of publishing
// a preseason power ranking - see conversation, Sept 2026: waiting for
// Week 1 to display it defeats the purpose of having it). So this
// accepts a candidate once EITHER games OR schedule has rows - the
// latter is what add_season.py actually populates first, well before
// kickoff, once a season's slate is real. Only a bare preseason_ratings
// row with neither still falls through to the prior season.
export async function getCurrentSeason(league) {
  ...
  for (const season of seasons) {
    const [{ count: gameCount, error: gameError }, { count: scheduleCount, error: scheduleError }] = await Promise.all([
      supabase
        .from("games")
    ...
    if (!gameError && gameCount > 0) return { season, error: null };
    if (!scheduleError && scheduleCount > 0) return { season, error: null };
  }
...
export const RATING_COLOR_STOPS = RATING_STOPS;
...
// `.eq("season", ...)` filter — historically this was the single most
// expensive query on the site (150k+ rows for a league with 30 seasons of
// NBA history), superseded on the All-Time page by fetchAllTimeTeamSeasons
// below, which gets the same end result from a Postgres view instead of
// shipping every raw game row to the browser. Still used by the Team
// page's fetchTeamAllSeasonsGames (single-team scope, not this whole-
// league one, so it was never the bottleneck) — kept here, not removed.
...
// One row per (team, season) — final rating, RS record, RS-only rating —
// computed in Postgres by the all_time_team_seasons view instead of
// downloading every individual game row and reducing client-side. Same
// output shape buildAllTimeRows() produces from fetchAllTimeTeamGames, so
// this is a drop-in replacement for the All-Time page specifically: a
// ~900-row query instead of a 150k+ row one for a 30-team, 30-season league.
export async function fetchAllTimeTeamSeasons(league, variant) {
  const { rows, error } = await fetchAllPages((from, to, withCount) =>
    supabase
      .from("all_time_team_seasons")
      .select("team_id, season, final_rating, rs_w, rs_l, rs_rating", withCount ? { count: "exact" } : undefined)
      .eq("league", league)
      .eq("variant", variant)
      .order("season", { ascending: true })
      .range(from, to)
  ...
  return {
    rows: rows.map((r) => ({
      ...
      rsW: r.rs_w ?? 0,
      rsL: r.rs_l ?? 0,
      rsRating: r.rs_rating ?? null,
      finalRating: r.final_rating ?? null,
    })),
    error: null,
  };
}
